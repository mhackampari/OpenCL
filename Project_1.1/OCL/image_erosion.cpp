...
int Erosion::setup(sImage* inputImage, sImage* outputImage, cl_device_type dType)
{
...
void Erosion::erosion(float &tempo)
{
...
int	Erosion::setupKernel(std::string name){
...
int Erosion::runErosion()
{
	cl_int status = 0;

	// Set kernel arguments and run kernel
	if (runCLKernelsErosion() != SDK_SUCCESS)
	{
		return SDK_FAILURE;
	}
	return SDK_SUCCESS;
...
int Erosion::runCLKernelsErosion()
{

	// Set appropriate arguments to the kernel
	//Initializing workitems
	int M, res2;;
