...
public:
    ...
    Knapsack(int numelem, cl_device_id device, cl_platform_id platform) {
    generateRandomData(numelem);
	...
	checkError(err);
	queue = clCreateCommandQueue(context, device_id[0], CL_QUEUE_PROFILING_ENABLE, &err);
	checkError(err);
    }

    void createProgramAndBuild() {
	...
	if (err != CL_SUCCESS) {
	    char buffer[4096] = {""};
	    size_t length = 4096;
	    clGetProgramBuildInfo(program, device_id[0], CL_PROGRAM_BUILD_LOG, 4096 * sizeof (buffer), buffer, 0);
	    cout << string(buffer) << "\n";
